--------------------------------------------------------------------------
-- The fifteen puzzle, again
--
--   Separate model from visualization, as in Fifteen2.hs, but make the
--   positions be reactive, as in Fifteen1.hs.
--
-- Last modified Fri Jul 18 11:10:10 1997 by conal
--------------------------------------------------------------------------


module Fifteen3 where
...
puzzle moveFrom blankLoc =
  map (pieceMoves (moveFrom `suchThat` validLoc
                            `snapshot` blankLoc
                            `suchThat` legalPMove))
       startPieces
...
legalPMove :: PMove -> Bool
legalPMove ((x1,y1), (x2,y2))  =  abs (x1-x2) + abs (y1-y2) == 1

-- Nothing ever moves
puzzleTest1 picked blankLoc = map (const neverE) startPieces

-- Move everything to picked location
puzzleTest2 picked blankLoc = map movePiece startPieces
 where
   movePiece startLoc = picked ==> (startLoc `pair`)

-- Move picked location to (-1,0)
puzzleTest3 picked blankLoc = map movePiece startPieces
 where
   movePiece startLoc =
...
demo picker puzzle = disp imF
 where
   imF u = renderBoard board
    where
      board    = puzzle pick blankLoc
      pick     = picker blankLoc u
      blankLoc = blankLocFromBoard board
...
fif1 = map legalPMove [((1,2),(2,2)), ((1,2),(3,4)), ((-1,0), (0,0))]

fif2 u = renderBoard nothingBoard

-- One piece that moves after three seconds.
fif3 u = renderPiece (occsE [(startTime u + 3, (from, to))]) from
 where
  from = (0,0)
  to   = (cols-1,rows-1)
